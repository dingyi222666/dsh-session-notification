...
浏览器（系统级）通知**默认关闭**；打开开关时会先向浏览器申请授权（需要一次点击手势）。授权后，当事件所属的会话不是当前正在读的会话、或标签页在后台时，才会弹系统通知。**完成的会话，通知正文会带上它的最终回复文本**（最后一条 assistant 消息）；「测试通知」按钮可以在授权后立即验证系统通知通道。**默认不打扰你正在读的会话**——它的动静不会打断你；想要它也有提示的话，打开「当前会话也提醒」开关即可。
