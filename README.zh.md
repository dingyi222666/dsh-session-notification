...
每种类型都可以单独开关，也可以把提示音换成四种内置音效中的任意一种（或静音）。四种音效全部用 Web Audio 实时合成——不携带任何音频文件——整体音量用官方风格的滑块调节（0–100%）。播放链路带固定响度增益（约 +6 dB）和软限幅器，所有声音都明显更响且不失真；自定义音频走同一条链路。播放结束后共享 AudioContext 会立即挂起、下次响铃再恢复，闲置通知不会一直占着系统音频流（以及 macOS 的防睡眠断言）。
