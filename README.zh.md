...
每种类型都可以单独开关，也可以把提示音换成四种内置音效中的任意一种（或静音）。四种音效全部用 Web Audio 实时合成——不携带任何音频文件——整体音量用官方风格的滑块调节（0–100%）。播放链路带固定响度增益（约 +6 dB）和软限幅器，所有声音都明显更响且不失真；自定义音频走同一条链路。
...
**「只提醒主会话」默认开启**：通知只来自主会话，并行展开的一堆子会话（subagent）不会每个都响一次；想连子会话一起听，把它关掉即可。
...
- dsh 0.1.3-alpha.1：`@deepseek-ai/dsh-*` 类型经 tsconfig paths 从同级 dsh 检出（`../test-dingyi222666`）解析，待 0.1.3-alpha.1 各包发布到 npm 后移除（devDependencies 只安装最近发布的版本，用于构建工具链）。
